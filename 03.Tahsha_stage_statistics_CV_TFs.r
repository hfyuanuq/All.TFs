cc <- read.csv("All.TFs.in_Tahsha_stages_sw_normalized.counts.csv", header = TRUE)
head(cc)
dim(cc)

colData <- data.frame(
  Sample = colnames(cc)[-1],
  Group = factor(
    c(rep("t0.5", 9), rep("t1.5", 9), rep("t3", 9), rep("t5", 9), rep("t8", 9)),
    levels = c("t0.5", "t1.5","t3", "t5", "t8")
  )
)
...
write.csv(stats_summary, file="All.TFs.in_Tahshs_stages.statistics.csv")
...
## output the result
write.csv(stats_summary_wide, "count.tables/All.TFs_in.Tahsha_stages_summary.csv", row.names = FALSE)
...
write.csv(high_var_genes_with_group, "count.tables/Tahsha_stages_high_var_genes_with_group.csv", row.names = FALSE)
...
ggsave("cv_distribution_Tahsha_stages.pdf", width = 8, height = 6)
